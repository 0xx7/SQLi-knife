...
def scan_urls(urls):
    vulns_setected = []
    for webpage in urls:
        db = check_if_inj_possible(webpage)
        if db:
...
def get_sql_errors():
    try:
        with open('data/sql_errors.json', 'r') as f:
...
def check_html_for_sql_errors(html_page_source, sql_errors):
    ...
    for db_type, errors in sql_errors.items():
        ...
        for error_type in errors:
            if re.compile(error_type).search(html_page_source):
                return True, db_type
    return False, None


#check if url is vulnerable or not
def check_if_inj_possible(url):
    #get dict of SQL errors
    sql_errors = get_sql_errors()
    print("[MSG] scanning {}".format(url))
    #get parametes
        ...
        for payload in ('`;', '\\', "%27", "%%2727", "%25%27", "%60", "%5C", \
            ...
            if source:
                ...
                if vulnerable and db != None:
                    print "url {} is vulnerable".format(url)
                    return db
    else:
        return False
