...
from sqli_engine import SQLI_engine
from sqli_database import Database
from sqli_table import Table
    ...
    def scan_url(self, url):
        try:
            ...
            if urlparse(url).query:
                ...
                result = html_scanner.scan_urls([url])
                # if no vulnerables - ask to start crawling or not
                if not result:
                    print("\n[MSG] no SQL injection vulnerability found\n")
                    option = raw_input("Do you want to continue crawling? [y/n]")
                    # if user don`t want to crawl - return
                    if option.lower() == 'n':
                        return False
            else:
                print("Please use URL with parameters!")
                print('\033[0m')
                exit()
            ...
            if not result:
                ...
                if not result:
                    print("[MSG] no vulnerability found")
            return result
        except Exception as e:
            print "[ERR]: {}".format(e)
            print traceback.format_exc()
    ...
    def save_results(self, result):
        ...
        for k, v in result.items():
            ...
            if temp:
                ...
                with open("results/{}.csv".format(k), "wb") as f:
                    print 'Table saved to ' + 'results/{}.csv'.format(k)
                    writer = csv.writer(f)
                    writer.writerows(temp)


    def do_injection(self, url):
        """
        Method do do sql injection
        :param url: url with no parameters
        :return:
        """
        print 'Working on injection'
        ...
        print "tables: " + '\033[0m' + tables + '\033[93m'
        result = {}
        ...
        for table in tables.split(','):
            ...
            if columns:
                ...
                print "columns: " + '\033[0m' + columns + '\033[93m'
